...
## SimConnect Mode (MCP_MODE=simconnect)

Live simulator data via SimConnect SDK. Windows only. Requires MSFS 2020 or MSFS 2024 running with SimConnect enabled.

## Tool Overview

| Tool | Description |
|------|-------------|
| [`get_simvar_value`](#get_simvar_value) | Read a single simulation variable value |
...
**Description**: Read up to 20 live simulation variables in a single call. Per-item failures are embedded in the response array and do not abort the batch.
...
**Returns**: An array of result objects. Each object has:
...
**Description**: Transmit a named SimConnect client event to the running simulator.
...
**Description**: Return a snapshot of current simulator connection state and flight status. Never returns an error — returns `{"connected": false}` when the bridge is not connected, allowing clients to safely poll at any connection state.
